...
{   If PageControl1.ActivePage = tabAditivaValor then
   ...
   Begin
     ...
     Begin
        ShowMessage('A soma da redução está diferente da de acréscimo!!!');
        AvcGrid.SetFocus;
        Exit;
     End;
   End;
   ...
   Begin
      ShowMessage('A Soma dos Detalhes desta Emenda é Maior que a Emenda Geral!!'+#13+#13+
...
begin
   ...
   Begin
       ...
       Begin
          ShowMessage('Emenda Já Cadastrada!!!');
          txtemenda.Clear;
          txtemenda.SetFocus;
          {
          seleto := 'S';
...
begin
   ...
   Begin
      ShowMessage('Autorização Negada!!!');
      btRenovar.OnClick(nil);
      Exit;
   End;
...
begin
  ...
  '        When E.Status = 3 Then '+ '''3 - Em Discussão''' +
  '     End, '+
...
begin
   ...
   If (qryProcessado.Value = 'S') AND (qryStatus.Value = '1') then// condição
   BEGIN
   ...
   If (qryProcessado.Value = 'S') AND (qryStatus.Value = '2') then// condição
   BEGIN
      Dbgrid.Canvas.Font.Color:= clWhite; // coloque aqui a cor desejada
...
begin
   ...
   Begin
      ShowMessage('Emenda ainda não cadastrda nas Gerais.'+#13+
   ...
   Begin
       ...
       If TotEme_rvalor.Value >= ValorEmendaGeral  Then
       Begin
          ShowMessage('A Soma dos Detalhes desta Emenda é igual a Emenda Geral. Portanto, '+#13+
                      'não pode ser mais utilizado!'+#13+
                      'Valor da Emenda Geral: '+ FormatCurr('###,###,##0.00',ValorEmendaGeral)+#13+
                      'soma das Emendas: '+ FormatCurr('###,###,##0.00',TotEme_rvalor.Value));

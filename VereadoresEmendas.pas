...
begin
   ...
   Begin
      ShowMessage('Já Existe Vereador para esta Emenda');
...
begin
   ...
   if qyVE_.RecordCount > 0 then
   Begin
      ShowMessage('Vereador já Cadastrado nesta Emenda!!!');
      edCod.SetFocus;
      exit;
   End;
   ...
   Begin
      ShowMessage('O Número da Emenda tem que existir!!!');
      Exit;
   End;
   If Trim(edCod.Text) = '' Then
   Begin
   ...
   If qyVE_.RecordCount > 0 Then
   Begin
      ShowMessage('Já existe um Vereador para esta emenda.');
      cbPesq.SetFocus;
      Exit;
   End;


   if application.messageBox('Confirma a Inclusão do Registro ?','INCLUSÃO',
...
begin
   ...
   if application.messageBox('Confirma a Exclusão do Registro ?','EXCLUSÃO',
      MB_ICONQUESTION+MB_YESNO) = IDno then Exit;

   qyVE_.Delete;

   ShowMessage('Vereador Excluído para esta emenda!!!');

object frmOrgCatDesp: TfrmOrgCatDesp
  Left = 224
  Top = 220
  Width = 693
  Height = 479
  HorzScrollBar.Position = 62
  ...
  object qryCat: TADOQuery
    ...
    SQL.Strings = (
      'SELECT orgao, cat, catD, despesaD = e.descricao,'
      '       total = SUM(valor),'
      '       tesouro = SUM(tesouro),'
      '       outras = SUM(outrasF),'
      '       fisTotal = SUM(fiscal),'
      ...
      '      WHERE orgao = :codigo'
      '      GROUP BY orgao, cat, catD, e.descricao'
      '      ORDER BY cat')
    Left = 8
    Top = 8
  end

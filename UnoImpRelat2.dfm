object frmUnoImpRelat2: TfrmUnoImpRelat2
  Left = 154
  Top = 142
  BorderIcons = [biMaximize, biHelp]
  BorderStyle = bsNone
  ClientHeight = 631
  ClientWidth = 803
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  OnClose = FormClose
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object Shape1: TShape
    Left = 8
    Top = 8
    Width = 789
    Height = 619
    Brush.Style = bsClear
    Pen.Color = clAppWorkSpace
  end
  object Shape4: TShape
    Left = 376
    Top = 56
    Width = 412
    Height = 517
    Brush.Style = bsClear
    Pen.Color = clWhite
  end
  object Shape2: TShape
    Left = 11
    Top = 11
    Width = 784
    Height = 34
    Brush.Color = clSilver
    Pen.Color = clWhite
  end
  object Label1: TLabel
    Left = 16
    Top = 20
    Width = 314
    Height = 16
    Alignment = taCenter
    Caption = 'IMPRESS'#195'O DE RELAT'#211'RIOS DAS UNIDADES OR'#199'AMENT'#193'RIAS'
    Font.Charset = ANSI_CHARSET
    Font.Color = clMenu
    Font.Height = -11
    Font.Name = 'Trebuchet MS'
    Font.Style = [fsBold]
    ParentFont = False
    Transparent = True
  end
  object Shape3: TShape
    Left = 12
    Top = 579
    Width = 781
    Height = 45
    Brush.Color = clSilver
    Pen.Color = clWhite
  end
  object SpeedButton1: TSpeedButton
    Left = 764
    Top = 17
    Width = 23
    Height = 22
    Hint = 'Fechar'
    Glyph.Data = {
      
      EAEAEAEAEAEAEAEAEAEA}
    ParentShowHint = False
    ShowHint = True
    OnClick = SpeedButton1Click
  end
  object Bevel2: TBevel
    Left = 16
    Top = 601
    Width = 776
    Height = 3
  end
  object Shape6: TShape
    Left = 383
    Top = 472
    Width = 399
    Height = 97
    Brush.Color = clScrollBar
    Pen.Color = clWhite
  end
  object Bevel1: TBevel
    Left = 384
    Top = 371
    Width = 393
    Height = 77
  end
  object RichEdit1: TRichEdit
    Left = 391
    Top = 480
    Width = 383
    Height = 81
    BorderStyle = bsNone
    BorderWidth = 4
    Color = cl3DLight
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clMaroon
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Lines.Strings = (
      'ATEN'#199#195'O !!! '
      ''
      
        'Ao clicar em qualquer bot'#227'o o sistema mandar'#225' imprimir o relat'#243'r' +
        'io de todas '
      'as unidades or'#231'ament'#225'rias.')
    ParentFont = False
    ReadOnly = True
    TabOrder = 0
  end
  object RgTipo_: TRadioGroup
    Left = 384
    Top = 276
    Width = 399
    Height = 57
    Caption = 'Tipos de Impress'#227'o'
    Columns = 2
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Trebuchet MS'
    Font.Style = [fsBold]
    ItemIndex = 0
    Items.Strings = (
      'Visualizar'
      'Imprimir')
    ParentFont = False
    TabOrder = 1
  end
  object CkB_: TCheckListBox
    Left = 16
    Top = 56
    Width = 353
    Height = 516
    BevelOuter = bvSpace
    BevelKind = bkSoft
    BorderStyle = bsNone
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Trebuchet MS'
    Font.Style = []
    ItemHeight = 16
    ParentFont = False
    TabOrder = 2
    OnClick = CkB_Click
  end
  object btTodos: TdxButton
    Left = 24
    Top = 591
    Width = 130
    Cursor = crHandPoint
    About = 'Design eXperience. '#169' 2002 M. Hoffmann'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -9
    Font.Name = 'Trebuchet MS'
    Font.Style = [fsBold]
    ParentFont = False
    Version = '1.0.2e'
    OnClick = btTodosClick
    Caption = 'Selecionar Todos [F2]'
    TabOrder = 3
  end
  object btTirar: TdxButton
    Left = 166
    Top = 591
    Width = 130
    Cursor = crHandPoint
    About = 'Design eXperience. '#169' 2002 M. Hoffmann'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -9
    Font.Name = 'Trebuchet MS'
    Font.Style = [fsBold]
    ParentFont = False
    Version = '1.0.2e'
    OnClick = btTirarClick
    Caption = 'Tirar Sele'#231#227'o [F3]'
    TabOrder = 4
  end
  object ckSelecao: TCheckBox
    Left = 384
    Top = 341
    Width = 393
    Height = 17
    Caption = 'Retirar Sele'#231#227'o ap'#243's impress'#227'o'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Trebuchet MS'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 5
  end
  object chkRelato: TCheckListBox
    Left = 385
    Top = 59
    Width = 394
    Height = 192
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Trebuchet MS'
    Font.Style = []
    ItemHeight = 16
    Items.Strings = (
      'Org'#227'o - Quadro S'#237'ntese - Fun'#231#227'o, SubFun'#231#227'o e Programa'
      'Org'#227'o -Total por Fonte/Grupo de Despesas'
      'Org'#227'o - S'#237'ntese por Unidade Or'#231'ament'#225'ria'
      
        'Org'#227'o - Demonstrativo da Despesa por Categoria Econ'#244'mica e Grupo' +
        ' de Despesas'
      'Unorc - Quadro S'#237'ntese - Fun'#231#227'o, SubFun'#231#227'o e Programa'
      'Unorc - Total por Fonte/Grupo de Despesas'
      
        'Unorc - Demonstrativo da Despesa por Categoria Econ'#244'mica e Grupo' +
        ' de Despesas'
      'Unorc - Detalhamento das A'#231#245'es'
      'Unorc - Quadro de Detalhamento da Despesa')
    ParentFont = False
    TabOrder = 6
  end
  object Button1: TButton
    Left = 387
    Top = 388
    Width = 386
    Height = 25
    Cursor = crHandPoint
    Caption = 'Gerar Relat'#243'rio'
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Trebuchet MS'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 7
    OnClick = Button1Click
  end
  object qry: TADOQuery
    Active = True
    Connection = dmtData.adoCon
    CursorType = ctStatic
    Parameters = <>
    Prepared = True
    SQL.Strings = (
      'SELECT u.*, orgaoS = SUBSTRING(u.codigo,1,2)+'#39'000'#39', '
      '       orgaoD  = o.descricao '
      '   FROM unorc u'
      '     INNER JOIN orgao o ON o.codigo = SUBSTRING(u.codigo,1,2)')
    Left = 24
    Top = 464
    object qrycodigo: TStringField
      FieldName = 'codigo'
      Size = 5
    end
    object qrydescricao: TStringField
      FieldName = 'descricao'
      Size = 255
    end
    object qryorgaoS: TStringField
      FieldName = 'orgaoS'
      ReadOnly = True
      Size = 5
    end
    object qryorgaoD: TStringField
      FieldName = 'orgaoD'
      Size = 255
    end
  end
  object sQry: TDataSource
    DataSet = qry
    Left = 56
    Top = 464
  end
  object ADOQuery1: TADOQuery
    Active = True
    Connection = dmtData.adoCon
    CursorType = ctStatic
    Parameters = <>
    Prepared = True
    SQL.Strings = (
      'SELECT u.*, orgaoS = SUBSTRING(u.codigo,1,2)+'#39'000'#39', '
      '       orgaoD  = o.descricao '
      '   FROM unorc u'
      '     INNER JOIN orgao o ON o.codigo = SUBSTRING(u.codigo,1,2)')
    Left = 32
    Top = 376
    object StringField1: TStringField
      FieldName = 'codigo'
      Size = 5
    end
    object StringField2: TStringField
      FieldName = 'descricao'
      Size = 255
    end
    object StringField3: TStringField
      FieldName = 'orgaoS'
      ReadOnly = True
      Size = 5
    end
    object StringField4: TStringField
      FieldName = 'orgaoD'
      Size = 255
    end
  end
  object DataSource1: TDataSource
    DataSet = ADOQuery1
    Left = 64
    Top = 376
  end
  object QRCompositeReport1: TQRCompositeReport
    OnAddReports = QRCompositeReport1AddReports
    Options = [FirstPageHeader, LastPageFooter, Compression]
    PrinterSettings.Copies = 2
    PrinterSettings.Duplex = False
    PrinterSettings.FirstPage = 0
    PrinterSettings.LastPage = 0
    PrinterSettings.OutputBin = Auto
    PrinterSettings.Orientation = poLandscape
    PrinterSettings.PaperSize = A4
    ReportTitle = 'pdf'
    Left = 752
    Top = 432
  end
end
